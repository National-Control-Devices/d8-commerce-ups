...
/**
 * @file
 * Contains \Drupal\ups\Form\UPSSettingsForm.
 */

namespace Drupal\ups\Form;
...
class UPSSettingsForm extends ConfigFormBase {
  ...
  public function getFormID() {
    return 'admin/commerce/config/shipping/methods/ups/edit_admin_settings';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    $ups_config = $this->config('ups.settings');
    ...
    $form['services']['ups_services'] = array(
      ...
      '#options' => \Drupal\ups\UPSUtilities::services(),
    ...
    $form['quote_options']['ups_all_in_one'] = array(
      '#type' => 'radios',
    ...
    $form['quote_options']['ups_package_type'] = array(
      '#type' => 'select',
    ...
    $form['quote_options']['ups_classification'] = array(
      '#type' => 'select',
    ...
    $form['quote_options']['ups_negotiated_rates'] = array(
      '#type' => 'radios',
    ...
    $form['quote_options']['ups_pickup_type'] = array(
      '#type' => 'select',
    ...
    $form['quote_options']['ups_residential_quotes'] = array(
      '#type' => 'radios',
    ...
    $form['quote_options']['ups_unit_system'] = array(
      '#type' => 'select',
    ...
    $form['quote_options']['ups_insurance'] = array(
      '#type' => 'checkbox',
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    $old_password = $this->config('ups.settings')->get('password');
    if (!$form_state->getValue('ups_password')) {
      if ($old_password) {
        $form_state->setValueForElement($form['ups_credentials']['ups_password'], $old_password);
      }
      else {
        $form_state->setErrorByName('ups_password', $this->t('Password field is required.'));
      }
    }

    if (!is_numeric($form_state->getValue('ups_rate_markup'))) {
      $form_state->setErrorByName('ups_rate_markup', $this->t('Rate markup must be a numeric value.'));
    }
    if (!is_numeric($form_state->getValue('ups_weight_markup'))) {
      $form_state->setErrorByName('ups_weight_markup', $this->t('Weight markup must be a numeric value.'));
    }
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $ups_config
      ->set('access_license', $values['ups_access_license'])
      ->set('shipper_number', $values['ups_shipper_number'])
      ->set('user_id', $values['ups_user_id'])
      ->set('password', $values['ups_password'])
      ->set('connection_address', $values['ups_connection_address'])
      ->set('services', $values['ups_services'])
      ->set('pickup_type', $values['ups_pickup_type'])
      ->set('package_type', $values['ups_package_type'])
      ->set('classification', $values['ups_classification'])
      ->set('negotiated_rates', $values['ups_negotiated_rates'])
      ->set('residential_quotes', $values['ups_residential_quotes'])
      ->set('rate_markup_type', $values['ups_rate_markup_type'])
      ->set('rate_markup', $values['ups_rate_markup'])
      ->set('weight_markup_type', $values['ups_weight_markup_type'])
      ->set('weight_markup', $values['ups_weight_markup'])
      ->set('label_lifetime', $values['ups_label_lifetime'])
      ->set('all_in_one', $values['ups_all_in_one'])
      ->set('unit_system', $values['ups_unit_system'])
      ->set('insurance', $values['ups_insurance'])
      ->save();
